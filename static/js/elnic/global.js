...
$(function() {
    ...
    $(".countdown")
      ...
      .each(function() {
        $(this)
          ...
          .ClassyCountdown({
            theme: $(this).hasClass("light")
              ? "light"
              : $(this).hasClass("light-green")
              ? "light-green"
              : "",
    ...
    $(".swiper-container")
      ...
      .each(function() {
        ...
        if ($t.find(">.swiper-button-prev").length) {
          $t.find(">.swiper-button-prev").addClass(
            "swiper-button-prev-" + index
          );
          $t.find(">.swiper-button-next").addClass(
        ...
        swipers["swiper-" + index] = new Swiper(".swiper-" + index, {
          ...
          autoHeight: $t.is("[data-auto-height]")
            ? parseInt($t.data("auto-height"), 10)
            : 0,
          loop: loopVar,
          autoplay: $t.is("[data-autoplay]")
            ? parseInt($t.data("autoplay"), 10)
            : 0,
          centeredSlides: $t.is("[data-center]")
            ? parseInt($t.data("center"), 10)
            : 0,
          ...
          parallax: $t.is("[data-parallax]")
            ? parseInt($t.data("parallax"), 10)
            : 0,
          slideToClickedSlide: $t.is("[data-click]")
            ? parseInt($t.data("click"), 10)
            : 0,
          ...
          direction: $t.is("[data-direction]")
            ? $t.data("direction")
            : "horizontal",
          preloadImages: false,
          lazyLoading: true,
          lazyLoadingInPrevNext:
            $t.data("direction") == "vertical" ? true : false,
          lazyLoadingInPrevNextAmount:
            $t.data("direction") == "vertical" ? 100 : 1,
  ...
  $(".toggle-search, .header-search-wrapper .button-close").on(
    "click",
    function() {
  ...
  $(document).on(
    "click",
    ".popup-wrapper .button-close, .popup-wrapper .layer-close",
    function(e) {
      e.preventDefault();
  ...
  $(document).on(
    ...
    function() {
      ...
      $(this)
        .closest(".product-preview-shortcode")
  ...
  $(document).on("click", ".quantity-select .minus", function() {
    ...
    $(this)
      .parent()
      .find(".number")
      .text(newValue > 1 ? newValue - 1 : newValue);
  });
  ...
  $(".cart-entry-description .button-close").on("click", function() {
    if (
      $(this)
        .closest(".cart-overflow")
        .find(".cart-entry").length == 1
    )
      $(this)
        .closest(".cart-entry")
        .replaceWith('<h4 class="h4">Your shopping cart is empty</h4>');
    else
      $(this)
        .closest(".cart-entry")
        .remove();
